...
export function SubscriptionManager() {
  ...
  const renderExpirationBadge = () => {
    ...
    if (status.estaVencida) {
      return (
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-red-500/10 border border-red-500/30 text-red-600 dark:text-red-300 text-xs animate-pulse">
    ...
    if (dias <= 7) {
      return (
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-300 text-xs">
    ...
    return (
      <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-teal-500/10 border border-teal-500/20 text-teal-600 dark:text-teal-400 text-xs font-medium">
        <svg className="w-3.5 h-3.5 shrink-0 text-teal-500" fill="currentColor" viewBox="0 0 20 20">
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-muted-foreground space-y-3">
        ...
        <p className="text-xs font-medium">Cargando detalles de facturación...</p>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="space-y-6 animate-in fade-in duration-200">
        {error && (
          <div className="p-4 bg-red-500/10 border border-red-500/25 rounded-2xl text-red-500 dark:text-red-300 text-xs">
            {error}
          </div>
        )}
        ...
        {status && (
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-1 p-6 rounded-2xl bg-card border border-border shadow-xs flex flex-col justify-between gap-6">
                <div className="space-y-1">
                  ...
                  <h3 className="text-lg font-bold text-foreground pt-1">{status.planNombre}</h3>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-bold text-foreground">
                      ...
                      <span className="text-muted-foreground/60 font-normal">/ {status.limiteUsuarios}</span>
                    </span>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {PLANES.map((plan) => {
                    ...
                    return (
                      <div
                        key={plan.id}
                        className={`relative p-5 rounded-2xl border flex flex-col justify-between gap-5 transition-all shadow-xs ${
                          isActive
                      ...
                      >
                        {isPremium && (
                          <div className="absolute -top-2.5 left-4">
                            <span className="text-[9px] font-black uppercase tracking-wider bg-gradient-to-r from-amber-400 to-orange-400 text-black px-2.5 py-1 rounded-full shadow-xs">
                              Más Popular
                            </span>
                          </div>
                        )}
                        ...
                        <div>
                          <div className="flex justify-between items-start">
                            ...
                            {isActive && (
                              <span className="text-[9px] font-bold bg-primary text-primary-foreground px-2 py-0.5 rounded uppercase tracking-wider">
                                Activo
                              </span>
                            )}
                          </div>
                          ...
                          <div className="mt-3 flex items-baseline gap-1.5">
                            <span className={`text-2xl font-extrabold ${isPremium ? 'text-amber-500' : 'text-foreground'}`}>
                ...
                <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-card border border-border">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {['PSE', 'Nequi', 'Bancolombia', 'Tarjeta Crédito', 'Tarjeta Débito'].map((m) => (
                      <span
                        key={m}
                        className="text-[9px] font-bold uppercase tracking-wider text-muted-foreground bg-secondary border border-border px-2 py-0.5 rounded"
                      >
                        {m}
                      </span>
                    ))}
      ...
      {showBuyModal && selectedPlan && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4">
          <div className="bg-card border border-border rounded-2xl w-full max-w-md p-6 relative animate-in zoom-in-95 duration-200 shadow-2xl text-foreground">
            <button
            ...
            <div className="mb-6">
              <h3 className="text-base font-extrabold text-foreground">Adquirir {selectedPlan.name}</h3>
                    ...
                    <button
                      ...
                      className={`relative py-4 px-2 rounded-xl border text-center transition-all duration-200 cursor-pointer flex flex-col items-center justify-center gap-0.5 ${
                        isSelected
                          ...
                          : 'bg-background border-border text-muted-foreground hover:border-primary/50 hover:text-foreground hover:bg-secondary'
                      }`}
                    >
                      ...
                      {desc > 0 && (
                        <span className="absolute -top-1.5 -right-1 text-[8px] font-black bg-emerald-500 text-white px-1.5 py-0.5 rounded-full shadow-xs leading-none">
                          -{desc}%
                        </span>
                      )}
                    </button>
            ...
            {/* Resumen de precio */}
            <div className="rounded-xl bg-background border border-border p-4 mb-6 space-y-3">
              ...
              <div className="pt-3 border-t border-border flex justify-between items-baseline">
                ...
                <span className="text-xl font-black text-foreground">{formatCOP(precioFinal * 100)}</span>
              </div>
            </div>

            {paymentError && (
              <div className="mb-4 p-3 bg-red-500/10 border border-red-500/25 rounded-xl text-red-500 dark:text-red-300 text-xs">
                {paymentError}
              </div>
            )}
            ...
            <button
              ...
              className="w-full py-3.5 bg-gradient-to-r from-indigo-600 to-cyan-600 hover:from-indigo-500 hover:to-cyan-500 rounded-xl text-sm font-black text-white transition-all shadow-md active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer"
            >
              {generatingParams ? (
                <>
